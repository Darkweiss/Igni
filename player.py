...
class player:
    ...
    def update(self,tilemap,offset,movement=(0,0)):        
        ...
        for rect in tilemap.physics_rects_around(self.pos,offset):
        ...
        entity_rect = self.rect_draw(offset)
        for rect in tilemap.physics_rects_around(self.pos,offset):
            if entity_rect.colliderect(rect):
    ...
    def rect_draw(self,offset):
        return  pygame.Rect(self.pos[0],self.pos[1] - offset,self.size[0],self.size[1])
